...
public:
                              ...
                              std::vector<double>& local_Jac_data) override
    {
        ...
        for (unsigned ip = 0; ip < n_integration_points; ip++)
        {
            ...
            double const scalar = (Invariants::identity2).transpose() * C_tensile * Invariants::identity2;
            //
            ...
            local_rhs.template block<phasefield_size, 1>(phasefield_index, 0)
               .noalias() -=
               N.transpose() * d_ip * alpha * T0 * scalar * alpha * T0 * w;
            ...
            for (Eigen::MatrixXd::Index i = 0; i < local_matrix_size; i++)
            {
                ...
                double const degradation_p = d_ip_p * d_ip_p + k;
                double delta_T_p(T_ip_p - T0);
                double rho_s_p = rho_sr * (1 - 3 * alpha * delta_T_p);
                _ip_data[ip].updateConstitutiveRelation(t, x_position, dt,
                                                        u_p, alpha, delta_T_p, degradation_p);
                double const scalar_p = (Invariants::identity2).transpose() * C_tensile * Invariants::identity2;
                local_b_p
                    .template block<displacement_size, 1>(displacement_index, 0)
                    .noalias() =
                    (B.transpose() * sigma_real
                    ...
                    + B.transpose() * ((d_ip*d_ip + k) * C_tensile + C_compressive) *
                        alpha * T0 * Invariants::identity2 * w
                    - B.transpose() * ((d_ip*d_ip + k) * C_tensile + C_compressive) *
                        alpha * Invariants::identity2 * N * T_p * w;
                    // + B.transpose() * 2 * d_ip_p * sigma_tensile * N * d_p * w;
                local_b_p
                   .template block<phasefield_size, 1>(phasefield_index, 0)
                   .noalias() =
                   (N.transpose() * d_ip_p / dt / M +
                    dNdx.transpose() * 2 * gc * ls * dNdx * d_p +
                ...
                local_b_p.template block<phasefield_size, 1>(phasefield_index, 0)
                   .noalias() =
                   N.transpose() * d_ip_p * alpha * T0 * scalar_p * alpha * T0 * w;
                local_b_p
                   .template block<phasefield_size, 1>(phasefield_index, 0)
                   .noalias() =
                   - N.transpose() * 2 * d_ip_p * scalar_p * alpha * alpha * T_ip_p * N * T_p * w;
                // double const eps_trace_p = Invariants::trace(eps);
                /*if (eps_trace_p >= 0)
                {
                    local_b_p
                       .template block<temperature_size, 1>(temperature_index, 0)
                       .noalias() = dNdx.transpose() * (d_ip_p*d_ip_p + k)
                       * lambda * dNdx * T_p * w
                       + N.transpose() * rho_s_p * c * N * T_p / dt * w;
                       // + dNdx.transpose() * 2 * d_ip_p * lambda * T_ip_p * dNdx * d_p * w;
                }
                else
                {
                    local_b_p
                       .template block<temperature_size, 1>(temperature_index, 0)
                       .noalias() = dNdx.transpose() * lambda * dNdx * T_p * w
                       + N.transpose() * rho_s_p * c * N * T_p / dt * w;
                }*/
                local_b_p
                   .template block<temperature_size, 1>(temperature_index, 0)
                ...
                double const degradation_m = d_ip_m * d_ip_m + k;
                double delta_T_m(T_ip_m - T0);
                double rho_s_m = rho_sr * (1 - 3 * alpha * delta_T_m);
                _ip_data[ip].updateConstitutiveRelation(t, x_position, dt,
                                                        u_m, alpha, delta_T_m, degradation_m);
                double const scalar_m = (Invariants::identity2).transpose() * C_tensile * Invariants::identity2;
                local_b_m
                    .template block<displacement_size, 1>(displacement_index, 0)
                    .noalias() =
                    (B.transpose() * sigma_real
                    ...
                    + B.transpose() * ((d_ip*d_ip + k) * C_tensile + C_compressive) *
                        alpha * T0 * Invariants::identity2 * w
                    - B.transpose() * ((d_ip*d_ip + k) * C_tensile + C_compressive) *
                        alpha * Invariants::identity2 * N * T_m * w;
                    // + B.transpose() * 2 * d_ip_m * sigma_tensile * N * d_m * w;
                local_b_m
                   .template block<phasefield_size, 1>(phasefield_index, 0)
                   .noalias() =
                   (N.transpose() * d_ip_m / dt / M +
                    dNdx.transpose() * 2 * gc * ls * dNdx * d_m +
                ...
                local_b_m.template block<phasefield_size, 1>(phasefield_index, 0)
                   .noalias() =
                   N.transpose() * d_ip_m * alpha * T0 * scalar * alpha * T0 * w;
                local_b_m
                   .template block<phasefield_size, 1>(phasefield_index, 0)
                   .noalias() =
                   - N.transpose() * 2 * d_ip_m * scalar * alpha * alpha * T_ip_m * N * T_m * w;
                // double const eps_trace_m = Invariants::trace(eps);
                /*if (eps_trace_m >= 0)
                {
                    local_b_m
                       .template block<temperature_size, 1>(temperature_index, 0)
                       .noalias() = dNdx.transpose() * (d_ip_m*d_ip_m + k)
                       * lambda * dNdx * T_m * w
                       + N.transpose() * rho_s_m * c * N * T_m / dt * w;
                       // + dNdx.transpose() * 2 * d_ip_m * lambda * T_ip_m * dNdx * d_m * w;
                }
                else
                {
                    local_b_m
                       .template block<temperature_size, 1>(temperature_index, 0)
                       .noalias() = dNdx.transpose() * lambda * dNdx * T_m * w
                       + N.transpose() * rho_s_m * c * N * T_m / dt * w;
                }*/
                local_b_m
                   .template block<temperature_size, 1>(temperature_index, 0)
        ...
        std::cout << "Jacobian" << local_Jac << "\n";
        std::cout << "residual" << local_rhs << "\n";
